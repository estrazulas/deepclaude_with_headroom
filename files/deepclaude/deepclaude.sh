...
# Usage: deepclaude [--backend ds|or|fw|anthropic] [--headroom] [--remote] [--status] [--cost] [--benchmark]
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --headroom|-H) HEADROOM_MODE=true; shift ;;
        --headroom-url) HEADROOM_URL="$2"; shift 2 ;;
...
show_help() {
    ...
    echo "  -H, --headroom                      Route through headroom proxy (default: http://localhost:8787)"
    echo "  --headroom-url <url>                Custom headroom proxy URL"
...
launch_claude() {
    ...
    if $HEADROOM_MODE; then
        export ANTHROPIC_BASE_URL="$HEADROOM_URL"
        echo "  Headroom proxy: $HEADROOM_URL -> $RESOLVED_URL"
    fi
